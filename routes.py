...
def packet1(packet):
    ...
    tempC = []
    tempN = []
    if os.path.getsize("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/noa.json") == 0:
        for i1 in packet:
            if i1["MessageType"] == "NewOrderAcknowledged":
                tempN.append(i1)
        with open("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/noa.json", "w") as r:
            r.write(json.dumps(tempN))

    f1 = open("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/noa.json")
    data1 = json.load(f1)
    f1.close()

    if os.path.getsize("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/cancelled.json") == 0:
        for i1 in packet:
            if i1["MessageType"] == "Cancelled":
                tempC.append(i1)
        with open("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/cancelled.json", "w") as r:
            r.write(json.dumps(tempC))

    f2 = open("/Users/brahimhamidoudjana/PycharmProjects/APIinterface/APISystem/P29R0/cancelled.json")
    data2 = json.load(f2)
    f2.close()

    for m in packet:
        if m["MessageType"] == "NewOrderAcknowledged":
            data1.append(m)

    for m1 in packet:
        if m1["MessageType"] == "Cancelled":
            data2.append(m)

    if not os.path.isdir("./P29R0"):
        os.makedirs("./P29R0")

    dumpJSON("noa.json", "P29R0", data1, "w")
    dumpJSON("cancelled.json", "P29R0", data2, "w")
...
def stock(ticker, packet_number):
    ...
    if current_user.is_authenticated:
        ...
        response = make_response(
            # packet,
            [{"msg1": packet}, {"msg2": packet1(packet)}],
            200
        )
...
def page():
    return render_template("charts.html", title='stocks')
